...
# ok, fbans dropped!
for dependenciesRequiredForTheBuild in java python3 zip lz4 tar file unzip simg2img make xmlstarlet mkfs.erofs mkdtimg imjtool aria2c openssl; do
	...
	if stringFormat --lower "$(file $argOne)" | grep -q zip; then
		if unzip -l "$argOne" | grep -qE "AP_|HOME_CSC"; then
			...
			for patchableImages in recovery.img.lz4 dtbo.img.lz4; do
				unzip -o "${extractedAPFilePath}" "${patchableImages}" -d ./local_build/etc/extract/ &>>${thisConsoleTempLogFile} || abort "Failed to extract ${patchableImages} from the archive." "build.sh"
...
[[ $TARGET_REMOVE_NONE_SECURITY_OPTION == true || $TARGET_REMOVE_SWIPE_SECURITY_OPTION == true ]] && buildAndSignThePackage "${DECODEDAPKTOOLPATHS[0]}" "$TSUKIKA_FALLBACK_OVERLAY_PATH"
...
if [ $TARGET_INCLUDE_TSUKIKA_OEMCRYPTO_DISABLER == true ]; then
	touch "./local_build/tmp/tsuki/liboemcrypto.so" # let's not make it "touch" everytime when it's in a loop.
	for libdir in "${SYSTEM_DIR}/lib" "${VENDOR_DIR}/lib64"; do
		[ -f "$libdir/liboemcrypto.so" ] && cp -a "./local_build/tmp/tsuki/liboemcrypto.so" "$libdir/liboemcrypto.so"
	done
fi
...
if [ $CUSTOM_WALLPAPER_RES_JSON_GENERATOR == true ]; then
	...
	clear
	rm -rf ./src/tsukika/packages/flosspaper_purezza/raw/resources_info.json
...
[[ ${BUILD_TARGET_SDK_VERSION} == 35 && -n "${roynaWhat}" ]] && buildAndSignThePackage "${DECODEDAPKTOOLPATHS[4]}" "$TSUKIKA_FALLBACK_OVERLAY_PATH"
...
if [ ${TARGET_BUILD_ADD_SCREENRESOLUTION_CHANGER} == true ]; then
	...
	if [[ ${BUILD_TARGET_SCREEN_WIDTH} =~ ^[0-9]+$ && ${BUILD_TARGET_SCREEN_HEIGHT} =~ ^[0-9]+$ && ${BUILD_TARGET_SCREEN_WIDTH} -eq 1080 && ${BUILD_TARGET_SCREEN_HEIGHT} -eq 2340 ]]; then
		...
		tar -C "./src/tsukika/packages/" -xf "${DECODEDAPKTOOLPATHS[5]}.tar"
...
if [ $TARGET_BUILD_CUSTOMIZE_SETUP_WIZARD_STRINGS == true ]; then
	logInterpreter "Unpacking Android ${BUILD_TARGET_ANDROID_VERSION} framework..." "apktool install-framework ${SYSTEM_DIR}/framework/framework-res.apk" || abort "Failed to unpack framework app."
	...
	xmlstarlet sel -t -v "//string[@name='outro_title']" ./src/tsukika/overlay_packages/setupWizard/res/values/strings.xml | grep -q '^".*"$' && \
		...
		xmlstarlet ed -L -u '//string[@name="outro_title"]' -v "${TARGET_BUILD_SETUP_WIZARD_OUTRO_TEXT}" ./src/tsukika/overlay_packages/setupWizard/res/values/strings.xml
	logInterpreter "Building overlay package..." "buildAndSignThePackage ./src/tsukika/overlay_packages/setupWizard ${TSUKIKA_FALLBACK_OVERLAY_PATH}/" || abort "Failed to build overlay package."
fi
...
for defaultTsukikaAlertSounds in "ro.config.ringtone whatever.ogg" "ro.config.ringtone_2 whatever.ogg" "ro.config.notification_sound Bling.ogg" "ro.config.notification_sound_2 Bling.ogg"; do
